...
function renderLoginHTML() {
	return `<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>管理后台登录 - EdgeTunnel</title><style>:root{--bg-base:#0f172a;--card-bg:rgba(30,41,59,0.85);--accent:#f6821f;--accent-hover:#faab41;--text-main:#f8fafc;--text-sub:#94a3b8;--border:rgba(255,255,255,0.1);--error:#ef4444}*{margin:0;padding:0;box-sizing:border-box}body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif;min-height:100vh;display:flex;align-items:center;justify-content:center;background:radial-gradient(circle at top,#1e293b 0%,#0f172a 100%);color:var(--text-main);padding:20px}.login-card{background:var(--card-bg);border:1px solid var(--border);backdrop-filter:blur(20px);padding:40px;border-radius:20px;width:100%;max-width:400px;box-shadow:0 25px 50px -12px rgba(0,0,0,0.5);text-align:center}.logo-icon{width:64px;height:64px;margin:0 auto 20px;background:linear-gradient(135deg,var(--accent) 0%,var(--accent-hover) 100%);border-radius:16px;display:flex;align-items:center;justify-content:center}.logo-icon svg{width:34px;height:34px;fill:white}h1{font-size:24px;font-weight:700;margin-bottom:8px}p.sub{font-size:14px;color:var(--text-sub);margin-bottom:28px}.form-group{margin-bottom:20px;text-align:left}label{display:block;font-size:13px;color:var(--text-sub);margin-bottom:8px}input[type="password"]{width:100%;padding:14px 16px;background:rgba(15,23,42,0.6);border:1px solid var(--border);border-radius:12px;color:#fff;font-size:16px;outline:none}input[type="password"]:focus{border-color:var(--accent);box-shadow:0 0 0 3px rgba(246,130,31,0.2)}.btn-submit{width:100%;padding:14px;background:linear-gradient(135deg,var(--accent) 0%,var(--accent-hover) 100%);border:none;border-radius:12px;color:#fff;font-size:16px;font-weight:600;cursor:pointer}.error-msg{color:var(--error);font-size:13px;margin-top:12px;display:none}</style></head><body><div class="login-card" id="card"><div class="logo-icon"><svg viewBox="0 0 24 24"><path d="M12 1L3 5v6c0 5.55 3.84 10.74 9 12 5.16-1.26 9-6.45 9-12V5l-9-4zm0 10.99h7c-.53 4.12-3.28 7.79-7 8.94V12H5V6.3l7-3.11v8.8z"/></svg></div><h1>管理后台登录</h1><p class="sub">EdgeTunnel 本地管理面板</p><form id="loginForm"><div class="form-group"><label for="password">管理员密码 (ADMIN / UUID / KEY)</label><input type="password" id="password" name="password" placeholder="请输入管理员密码" required autofocus></div><button type="submit" class="btn-submit" id="submitBtn">登录系统</button><div class="error-msg" id="errorMsg">密码错误，请重试</div></form></div><script>const form=document.getElementById('loginForm'),btn=document.getElementById('submitBtn'),err=document.getElementById('errorMsg');form.addEventListener('submit',async(e)=>{e.preventDefault();btn.disabled=true;btn.textContent='登录中...';err.style.display='none';try{const res=await fetch('/login',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:new URLSearchParams({password:document.getElementById('password').value}).toString()});const data=await res.json();if(data&&data.success)window.location.href='/admin';else throw new Error();}catch(e){err.style.display='block';btn.disabled=false;btn.textContent='登录系统';}});</script></body></html>`;
}

function renderNoAdminHTML() {
	return `<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><title>未配置管理员密码 - EdgeTunnel</title><style>body{font-family:sans-serif;background:#0f172a;color:#fff;display:flex;align-items:center;justify-content:center;min-height:100vh;padding:20px}.card{background:#1e293b;padding:36px;border-radius:18px;max-width:480px;text-align:center;border:1px solid rgba(255,255,255,0.1)}h1{color:#ef4444;font-size:22px;margin-bottom:14px}p{color:#94a3b8;font-size:14px;line-height:1.6;margin-bottom:16px;text-align:left}.code{background:#0f172a;padding:12px;border-radius:8px;font-family:monospace;color:#38bdf8;margin-bottom:18px;text-align:left}</style></head><body><div class="card"><h1>未设置管理员密码 (ADMIN)</h1><p>请在 Cloudflare 控制台添加环境变量：</p><div class="code">变量名称: ADMIN<br>变量内容: 你的自定义管理密码</div><p>添加后重新部署即可登录后台。</p></div></body></html>`;
}

function renderNoKvHTML() {
	return `<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><title>未绑定 KV 数据库 - EdgeTunnel</title><style>body{font-family:sans-serif;background:#0f172a;color:#fff;display:flex;align-items:center;justify-content:center;min-height:100vh;padding:20px}.card{background:#1e293b;padding:36px;border-radius:18px;max-width:480px;text-align:center;border:1px solid rgba(255,255,255,0.1)}h1{color:#f59e0b;font-size:22px;margin-bottom:14px}p{color:#94a3b8;font-size:14px;line-height:1.6;margin-bottom:16px;text-align:left}.code{background:#0f172a;padding:12px;border-radius:8px;font-family:monospace;color:#38bdf8;margin-bottom:18px;text-align:left}</style></head><body><div class="card"><h1>未绑定 KV 命名空间</h1><p>管理后台需要绑定 Cloudflare KV 命名空间：</p><div class="code">变量/绑定名称: KV<br>KV 命名空间: 选择或创建一个 KV 命名空间</div><p>在控制台「设置」->「函数」->「KV 命名空间绑定」中绑定后重新部署。</p></div></body></html>`;
}

function renderAdminHTML() {
	return `<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>EdgeTunnel 本地管理后台</title><style>:root{--bg-base:#0b0f19;--bg-card:#151d30;--bg-input:#1e293b;--text-primary:#f8fafc;--text-secondary:#94a3b8;--border:rgba(255,255,255,0.08);--accent:#f6821f;--accent-hover:#faab41;--accent-glow:rgba(246,130,31,0.35);--success:#10b981;--danger:#ef4444;--radius:14px}*{margin:0;padding:0;box-sizing:border-box}body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif;background-color:var(--bg-base);color:var(--text-primary);min-height:100vh;display:flex;flex-direction:column}.navbar{height:68px;padding:0 28px;display:flex;align-items:center;justify-content:space-between;background:rgba(21,29,48,0.7);backdrop-filter:blur(16px);border-bottom:1px solid var(--border);position:sticky;top:0;z-index:100}.brand{display:flex;align-items:center;gap:12px;font-weight:700;font-size:18px;color:#fff}.brand-icon{width:38px;height:38px;background:linear-gradient(135deg,var(--accent) 0%,var(--accent-hover) 100%);border-radius:10px;display:flex;align-items:center;justify-content:center;box-shadow:0 4px 12px var(--accent-glow)}.brand-icon svg{width:22px;height:22px;fill:#fff}.nav-actions{display:flex;align-items:center;gap:12px}.btn{padding:8px 16px;border-radius:10px;border:1px solid var(--border);background:var(--bg-input);color:var(--text-primary);font-size:14px;font-weight:500;cursor:pointer;display:inline-flex;align-items:center;gap:6px;transition:all 0.2s}.btn:hover{background:rgba(255,255,255,0.08);border-color:rgba(255,255,255,0.2)}.btn-primary{background:linear-gradient(135deg,var(--accent) 0%,var(--accent-hover) 100%);border:none;color:#fff;font-weight:600;box-shadow:0 4px 12px var(--accent-glow)}.btn-danger{background:rgba(239,68,68,0.15);color:var(--danger);border-color:rgba(239,68,68,0.3)}.btn-danger:hover{background:var(--danger);color:#fff}.main-container{flex:1;max-width:1200px;width:100%;margin:0 auto;padding:28px 20px;display:flex;flex-direction:column;gap:24px}.card{background:var(--bg-card);border:1px solid var(--border);border-radius:var(--radius);padding:24px;box-shadow:0 10px 30px rgba(0,0,0,0.25)}.card-header{display:flex;align-items:center;justify-content:space-between;margin-bottom:20px;padding-bottom:14px;border-bottom:1px solid var(--border)}.card-title{font-size:16px;font-weight:700;display:flex;align-items:center;gap:8px}.sub-banner{display:flex;gap:12px;align-items:center;background:rgba(15,23,42,0.6);border:1px solid var(--border);border-radius:12px;padding:12px 16px;margin-top:10px}.sub-url-box{flex:1;font-family:monospace;font-size:14px;color:#38bdf8;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.textarea-editor{width:100%;min-height:220px;background:var(--bg-input);border:1px solid var(--border);border-radius:10px;padding:14px;color:#f1f5f9;font-family:monospace;font-size:13px;line-height:1.6;resize:vertical;outline:none}.textarea-editor:focus{border-color:var(--accent)}.grid-2{display:grid;grid-template-columns:1fr 1fr;gap:24px}@media(max-width:840px){.grid-2{grid-template-columns:1fr}}.modal-overlay{position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.7);backdrop-filter:blur(8px);display:none;align-items:center;justify-content:center;z-index:1000;padding:20px}.modal-overlay.active{display:flex}.modal{background:#1e293b;border:1px solid var(--border);border-radius:20px;width:100%;max-width:540px;padding:28px;box-shadow:0 25px 50px -12px rgba(0,0,0,0.7)}.modal-header{display:flex;justify-content:space-between;align-items:center;margin-bottom:20px}.modal-title{font-size:18px;font-weight:700}.modal-close{background:none;border:none;font-size:24px;color:var(--text-secondary);cursor:pointer}.form-row{margin-bottom:16px}.form-label{display:block;font-size:13px;font-weight:600;color:var(--text-secondary);margin-bottom:6px}.form-input,.form-select{width:100%;padding:10px 14px;background:#0f172a;border:1px solid var(--border);border-radius:10px;color:#fff;font-size:14px;outline:none}.form-input:focus,.form-select:focus{border-color:var(--accent)}.chain-input-group{display:flex;gap:8px}.chain-select{width:110px;flex-shrink:0}.check-result-box{margin-top:12px;padding:10px 14px;border-radius:8px;background:rgba(15,23,42,0.6);font-size:13px;display:none}.toast{position:fixed;bottom:24px;right:24px;padding:12px 20px;border-radius:10px;background:#1e293b;border:1px solid var(--border);color:#fff;font-size:14px;box-shadow:0 10px 25px rgba(0,0,0,0.4);z-index:2000;transform:translateY(100px);opacity:0;transition:all 0.3s}.toast.show{transform:translateY(0);opacity:1}</style></head><body><nav class="navbar"><div class="brand"><div class="brand-icon"><svg viewBox="0 0 24 24"><path d="M12 1L3 5v6c0 5.55 3.84 10.74 9 12 5.16-1.26 9-6.45 9-12V5l-9-4zm0 10.99h7c-.53 4.12-3.28 7.79-7 8.94V12H5V6.3l7-3.11v8.8z"/></svg></div><span>EdgeTunnel 本地控制台</span></div><div class="nav-actions"><span style="font-size:13px;color:var(--success);display:flex;align-items:center;gap:6px"><span style="width:8px;height:8px;border-radius:50%;background:var(--success)"></span> 运行正常</span><a href="/logout" class="btn btn-danger" style="text-decoration:none">退出登录</a></div></nav><main class="main-container"><section class="card"><div class="card-header"><h2 class="card-title">📡 我的订阅地址</h2><button class="btn btn-primary" id="copySubBtn">复制订阅链接</button></div><p style="font-size:14px;color:var(--text-secondary)">将此订阅链接导入 Clash、v2rayN、Sing-box、Shadowrocket 等客户端即可自动获取所有节点配置：</p><div class="sub-banner"><div class="sub-url-box" id="subUrlDisplay">正在生成订阅链接...</div></div></section><section class="card"><div class="card-header"><h2 class="card-title">🔗 自定义优选节点与链式代理配置 (ADD.txt)</h2><div style="display:flex;gap:8px"><button class="btn" id="openChainModalBtn">➕ 添加链式代理节点</button><button class="btn btn-primary" id="saveAddBtn">💾 保存节点配置</button></div></div><p style="font-size:13px;color:var(--text-secondary);margin-bottom:12px">每行一个节点，格式支持：<code>优选域名/IP:端口#节点名称</code> 或带链式出站：<code>优选IP:443#香港节点 $vless://UUID@node.com:443?security=tls&type=ws</code></p><textarea class="textarea-editor" id="addTextEditor" placeholder="加载中..."></textarea></section><div class="grid-2"><section class="card"><div class="card-header"><h2 class="card-title">⚙️ 高级配置 (config.json)</h2><button class="btn btn-primary" id="saveConfigBtn">保存配置</button></div><textarea class="textarea-editor" id="configJsonEditor" style="min-height:260px" placeholder="加载中..."></textarea></section><section class="card"><div class="card-header"><h2 class="card-title">📝 访问日志 (log.json)</h2><button class="btn" id="refreshLogBtn">🔄 刷新</button></div><textarea class="textarea-editor" id="logViewer" readonly style="min-height:260px;color:#94a3b8" placeholder="暂无日志数据..."></textarea></section></div></main><div class="modal-overlay" id="chainModal"><div class="modal"><div class="modal-header"><h3 class="modal-title">🔗 添加链式代理节点</h3><button class="modal-close" id="closeChainModalBtn">&times;</button></div><div class="form-row"><label class="form-label">节点名称</label><input type="text" class="form-input" id="nodeNameInput" placeholder="例如：香港专线 01"></div><div class="form-row"><label class="form-label">优选入口 (Cloudflare 优选 IP 或 域名:端口)</label><input type="text" class="form-input" id="preferredHostInput" placeholder="例如：cf.zhahui.de5.net:443"></div><div class="form-row"><label class="form-label">链式代理出站协议与参数</label><div class="chain-input-group"><select class="form-select chain-select" id="proxyTypeSelect"><option value="vless" selected>vless</option><option value="socks5">socks5</option><option value="http">http</option><option value="https">https</option><option value="turn">turn</option><option value="sstp">sstp</option></select><input type="text" class="form-input" id="proxyParamInput" placeholder="UUID@hostname:443?security=tls&type=ws&path=/ws"></div><span style="font-size:12px;color:var(--text-secondary);margin-top:4px;display:block" id="proxyHintText">VLESS 格式: UUID@域名:端口?security=tls&type=ws&path=/ws&sni=域名</span></div><div class="check-result-box" id="checkResultBox"></div><div style="display:flex;gap:10px;margin-top:24px"><button class="btn" id="testProxyBtn" style="flex:1">⚡ 可用性验证</button><button class="btn btn-primary" id="confirmAddChainBtn" style="flex:1">➕ 添加到列表</button></div></div></div><div class="toast" id="toast"></div><script>function showToast(msg,isError=false){const t=document.getElementById('toast');t.textContent=msg;t.style.borderColor=isError?'var(--danger)':'var(--accent)';t.classList.add('show');setTimeout(()=>t.classList.remove('show'),3000)}async function initDashboard(){try{const host=window.location.host;fetch('/admin/ADD.txt').then(r=>r.text()).then(txt=>{document.getElementById('addTextEditor').value=txt==='null'?'':txt});fetch('/admin/config.json').then(r=>r.json()).then(cfg=>{document.getElementById('configJsonEditor').value=JSON.stringify(cfg,null,2)});loadLogs();document.getElementById('subUrlDisplay').textContent=\`\${window.location.origin}/sub\`;document.getElementById('preferredHostInput').value=\`\${host}:443\`}catch(e){showToast('加载数据失败: '+e.message,true)}}async function loadLogs(){try{const res=await fetch('/admin/log.json');const logs=await res.json();document.getElementById('logViewer').value=typeof logs==='string'?logs:JSON.stringify(logs,null,2)}catch(e){document.getElementById('logViewer').value='读取日志失败: '+e.message}}document.getElementById('copySubBtn').addEventListener('click',()=>{navigator.clipboard.writeText(document.getElementById('subUrlDisplay').textContent).then(()=>{showToast('订阅链接已成功复制到剪贴板！')})});document.getElementById('saveAddBtn').addEventListener('click',async()=>{try{const res=await fetch('/admin/ADD.txt',{method:'POST',headers:{'Content-Type':'text/plain;charset=utf-8'},body:document.getElementById('addTextEditor').value});if(res.ok)showToast('节点配置 (ADD.txt) 保存成功！');else throw new Error('保存失败')}catch(e){showToast(e.message,true)}});document.getElementById('saveConfigBtn').addEventListener('click',async()=>{try{JSON.parse(document.getElementById('configJsonEditor').value);const res=await fetch('/admin/config.json',{method:'POST',headers:{'Content-Type':'application/json'},body:document.getElementById('configJsonEditor').value});if(res.ok)showToast('高级配置 (config.json) 保存成功！');else throw new Error('保存配置失败')}catch(e){showToast('JSON 格式错误或保存失败: '+e.message,true)}});document.getElementById('refreshLogBtn').addEventListener('click',()=>{loadLogs();showToast('日志已刷新')});const modal=document.getElementById('chainModal');document.getElementById('openChainModalBtn').addEventListener('click',()=>modal.classList.add('active'));document.getElementById('closeChainModalBtn').addEventListener('click',()=>modal.classList.remove('active'));const proxyTypeSelect=document.getElementById('proxyTypeSelect'),proxyParamInput=document.getElementById('proxyParamInput'),proxyHintText=document.getElementById('proxyHintText');proxyTypeSelect.addEventListener('change',()=>{const t=proxyTypeSelect.value;if(t==='vless'){proxyParamInput.placeholder='UUID@hostname:443?security=tls&type=ws&path=/ws';proxyHintText.textContent='VLESS 格式: UUID@域名:端口?security=tls&type=ws&path=/ws&sni=域名'}else if(t==='socks5'){proxyParamInput.placeholder='username:password@1.2.3.4:1080';proxyHintText.textContent='SOCKS5 格式: 用户名:密码@IP:端口 或 IP:端口'}else{proxyParamInput.placeholder='username:password@1.2.3.4:8080';proxyHintText.textContent=\`\${t.toUpperCase()} 格式: 用户名:密码@IP:端口 或 IP:端口\`}});document.getElementById('testProxyBtn').addEventListener('click',async()=>{const type=proxyTypeSelect.value,param=proxyParamInput.value.trim(),resBox=document.getElementById('checkResultBox');if(!param){showToast('请先输入代理参数',true);return}resBox.style.display='block';resBox.innerHTML='<span style="color:#fbbf24">⏳ 正在探测连通性与出站 IP...</span>';try{const res=await fetch(\`/admin/check?\${type}=\${encodeURIComponent(param)}\`);const data=await res.json();if(data.ip){resBox.innerHTML=\`<span style="color:var(--success)">✅ 连通成功！延迟: \${data.time||data.delay||'正常'}ms<br>出站真实 IP: <b>\${data.ip}</b> (\${data.as||data.loc||''})</span>\`}else{resBox.innerHTML=\`<span style="color:var(--danger)">❌ 连接失败: \${data.error||'节点无响应'}</span>\`}}catch(e){resBox.innerHTML=\`<span style="color:var(--danger)">❌ 探测请求异常: \${e.message}</span>\`}});document.getElementById('confirmAddChainBtn').addEventListener('click',()=>{const name=document.getElementById('nodeNameInput').value.trim()||'链式代理节点',host=document.getElementById('preferredHostInput').value.trim()||window.location.host,type=proxyTypeSelect.value,param=proxyParamInput.value.trim();if(!param){showToast('请填写代理参数',true);return}const line=\`\${host}#\${name} $\${type}://\${param}\`,editor=document.getElementById('addTextEditor');editor.value=editor.value?editor.value.trim()+'\\n'+line:line;modal.classList.remove('active');showToast('已追加到节点列表，请点击「保存节点配置」生效！')});initDashboard();</script></body></html>`;
}

async function 获取静态页面响应(env, request, 页面路径, urlSearch = '') {
	const 规范路径 = 页面路径.startsWith('/') ? 页面路径 : '/' + 页面路径;
	// 1. Pages 模式：若有本地静态文件资产，优先从本地 asset 读取
	if (env.ASSETS && typeof env.ASSETS.fetch === 'function') {
		try {
			const reqUrl = new URL(request.url);
			reqUrl.pathname = 规范路径;
			const assetReq = new Request(reqUrl.toString() + urlSearch, request);
			const assetRes = await env.ASSETS.fetch(assetReq);
			if (assetRes && assetRes.status < 400) return assetRes;
		} catch (e) { }
	}
	// 2. 本地 KV 离线缓存：如果本地已缓存原版 HTML，100% 本地 0ms 瞬间返回
	if (env.KV && typeof env.KV.get === 'function') {
		try {
			const kvKey = `LOCAL_HTML_${规范路径.replace(/\//g, '_')}`;
			const cachedHtml = await env.KV.get(kvKey);
			if (cachedHtml) {
	...
	// 3. 固化原版 860KB 华丽完整 UI（带地图、仪表盘与动画）并写入本地 KV，彻底断绝后续外部依赖
	try {
		const targetUrl = `https://raw.githubusercontent.com/EDT-Pages/EDT-Pages.github.io/main${规范路径 === '/admin' || 规范路径 === '/login' || 规范路径 === '/noADMIN' || 规范路径 === '/noKV' ? 规范路径 + '/index.html' : 规范路径}`;
		const res = await fetch(targetUrl);
		if (res.ok) {
			let html = await res.text();
			if (规范路径.includes('admin')) {
				html = html.replace(/(<option\s+value=["']sstp["'][^>]*>.*?<\/option>)/gi, '$1<option value="vless">vless</option>');
			}
			if (env.KV && typeof env.KV.put === 'function') {
				await env.KV.put(`LOCAL_HTML_${规范路径.replace(/\//g, '_')}`, html);
			}
	...
	// 4. 本地内置轻量模板备用
	if (规范路径.includes('admin')) return new Response(renderAdminHTML(), { headers: { 'Content-Type': 'text/html;charset=utf-8' } });
	if (规范路径.includes('login')) return new Response(renderLoginHTML(), { headers: { 'Content-Type': 'text/html;charset=utf-8' } });
	if (规范路径.includes('noADMIN')) return new Response(renderNoAdminHTML(), { status: 404, headers: { 'Content-Type': 'text/html;charset=utf-8' } });
	if (规范路径.includes('noKV')) return new Response(renderNoKvHTML(), { status: 404, headers: { 'Content-Type': 'text/html;charset=utf-8' } });
...
export default {
	async fetch(request, env, ctx) {
		...
		if (访问路径 === 'version') {// 版本信息接口
			...
			if (url.protocol === 'http:' && !['localhost', '127.0.0.1', '0.0.0.0'].includes(url.hostname)) return Response.redirect(url.href.replace(`http://${url.hostname}`, `https://${url.hostname}`), 301);
			if (!管理员密码) return await 获取静态页面响应(env, request, '/noADMIN');
			if (env.KV && typeof env.KV.get === 'function') {
				...
				if (区分大小写访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') {//快速订阅
					...
					if (request.method === 'POST') {
						...
						if (输入密码 === (typeof 管理员密码 === 'string' ? 管理员密码.replace(/[\r\n]/g, '') : 管理员密码)) {
							...
							响应.headers.set('Set-Cookie', `auth=${await MD5MD5(UA + 加密秘钥 + 管理员密码)}; Path=/; Max-Age=86400; HttpOnly${cookieSecure}; SameSite=Lax`);
							return 响应;
						}
					}
					return await 获取静态页面响应(env, request, '/login');
					...
					return await 获取静态页面响应(env, request, '/admin', url.search);
			...
			} else if (!envUUID) return await 获取静态页面响应(env, request, '/noKV');
		}
